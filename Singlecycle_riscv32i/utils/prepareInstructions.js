...
if (textSection) {
	...
	console.log(`Prepared the instructions in file ${process.argv[2] + '.imem'}`)
	console.log(`Note: INSTMEM_LENGTH must be at least ${textBuffer.length > 200 ? textBuffer.length : 200}!`)
}
...
if (sections.length > 0) {
	...
	console.log(`Prepared the data in file ${process.argv[2] + '.dmem'}`)
	console.log(`Note: DATAMEM_LENGTH must be at least ${dmemBuffer.length+500}!`)
...
if (symtabSection) {
	for (let i = 0; i < symtabSection.symbols.length; i++) {
		...
		if (sym.name == '_start') {
			console.log(`Note: INITIAL_PC_ADDRESS must be ${Number(sym.value)}`)
			break
		}
	}
}
else {
	console.log('_start not found')
}
